...
<section id="popularcause" class="bg-gray waypoint-tigger xs-section-padding">
	<div class="container">
		...
		{#each $charities as charity}
			<div class="col-lg-4 col-md-6" in:slide={{delay: 1000}} out:fade={{delay: 1000}}> <!-- penggunaan animasi svelte transition -->
				<!-- bagian modal -->
			  {#if isModalOpen === true}
				<Modal>
					<h1>ini Modal</h1>
			  ...
			  {/if}
				...
				<div class="xs-popular-item xs-box-shadow d-flex">
...
<!-- <div>	
	...
	<ul>
		{#each charities as charity} melakuan looping foreach
		<li>{charity.title} - {charity.category}</li>
		{/each} akhir looping foreach
	</ul>
	{:else}
